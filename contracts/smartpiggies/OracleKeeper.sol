...
contract OracleKeeper {
    ...
    uint256 public immutable capacity = 200;
    ...
    function fulfillDataCall(bytes32 _requestId)
        internal
        returns (bool)
    {
        Request storage r = requests[_requestId];
        bytes memory payload = abi.encodeWithSelector(r.callback, _requestId, _data);
        ...
        // clean up the buffer, i.e. swap copy
        for(uint256 i = 0; i < capacity; i++)
        {
            if(index == tail)
            {
            ...
            if(index == 0) // crossed buffer capacity, reset index
            {
